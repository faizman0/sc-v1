...
local function setupDoubleDamage()
    pcall(function()
        -- Hook into RemoteEvents for damage
            ...
            if remoteEvent and remoteEvent:IsA("RemoteEvent") then
                ...
                remoteEvent.FireServer = function(self, ...)
                    local success, result = pcall(function()
                        ...
                        -- Check if this is a damage-related event
                        if doubleDamageConfig.enabled and args[1] and type(args[1]) == "table" then
                            -- Look for damage values in the arguments
                            for i, arg in pairs(args) do
                                if type(arg) == "table" then
                                    -- Check for damage-related keys
                                    if arg.damage or arg.Damage or arg.amount or arg.Amount then
                                        local damageKey = arg.damage and "damage" or arg.Damage and "Damage" or arg.amount and "amount" or "Amount"
                                        if arg[damageKey] and type(arg[damageKey]) == "number" then
                                            arg[damageKey] = arg[damageKey] * doubleDamageConfig.multiplier
                                        end
                                    end
                                elseif type(arg) == "number" and arg > 0 and arg < 1000 then
                                    -- If it's a number that looks like damage, multiply it
        ...
        -- Hook into RemoteFunctions for damage
            ...
            if remoteFunction and remoteFunction:IsA("RemoteFunction") then
                ...
                remoteFunction.InvokeServer = function(self, ...)
                    local success, result = pcall(function()
                        ...
                        -- Check if this is a damage-related function
                        if doubleDamageConfig.enabled and args[1] and type(args[1]) == "table" then
                            -- Look for damage values in the arguments
                            for i, arg in pairs(args) do
                                if type(arg) == "table" then
        ...
        -- Hook existing RemoteEvents and RemoteFunctions
        ...
        -- Hook new RemoteEvents and RemoteFunctions as they're added
