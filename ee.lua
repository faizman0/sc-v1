...
local Window = Rayfield:CreateWindow({
   Name = "Faiz Ni Boss Senggol Dong",
   Icon = 0,
   LoadingTitle = "Faiz",
   LoadingSubtitle = "by Faiz ni boss",
   Theme = "Default",
   ...
   KeySettings = {
      ...
      Key = {"FaizniBoss"}, -- list of keys that will open the script
   }
})
...
local aimbotConfig = {
    ...
    smoothing = 0.1,
    fov = 200,
    targetPart = "Head", -- SELALU HEAD
    teamCheck = true,
    visibleCheck = true,
    maxDistance = 1000,
    crosshairTargeting = true,
    dynamicHeadOffset = true, -- Offset dinamis berdasarkan jarak
    baseHeadOffset = 0.8, -- Offset dasar untuk jarak dekat
    maxHeadOffset = 0.3, -- Offset maksimum untuk jarak jauh
    extraHeight = 0.5, -- Extra height di atas kepala
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if input.UserInputType == Enum.UserInputType.MouseButton2 then
        ...
        aimbotConfig.rightMouseDown = true
    end
end)
...
UserInputService.InputEnded:Connect(function(input, gameProcessed)
    if input.UserInputType == Enum.UserInputType.MouseButton2 then
        ...
        aimbotConfig.rightMouseDown = false
    end
end)
...
local function simulateMouseMovement(deltaX, deltaY)
    -- Jangan gerakkan mouse jika klik kanan ditekan (untuk menggeser kamera)
    if aimbotConfig.rightMouseDown then
        return false
    end
...
-- Aimbot Functions - getClosestPlayer
local function getClosestPlayer()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.Health > 0 then
            ...
            -- Fallback jika Head tidak ditemukan
...
-- Fungsi untuk menghitung offset kepala dinamis berdasarkan jarak
local function calculateDynamicHeadOffset(distance)
    if not aimbotConfig.dynamicHeadOffset then
        return aimbotConfig.baseHeadOffset
    end
    
    -- Semakin jauh target, semakin kecil offset (crosshair lebih rendah)
    local minDistance = 10  -- Jarak minimum (sangat dekat)
    local maxDistance = 100 -- Jarak maksimum untuk perhitungan offset
    
    -- Clamp distance
    distance = math.clamp(distance, minDistance, maxDistance)
    
    -- Interpolasi linear antara baseHeadOffset dan maxHeadOffset
    local t = (distance - minDistance) / (maxDistance - minDistance)
    local offset = aimbotConfig.baseHeadOffset - (aimbotConfig.baseHeadOffset - aimbotConfig.maxHeadOffset) * t
    
    return math.clamp(offset, aimbotConfig.maxHeadOffset, aimbotConfig.baseHeadOffset)
end
...
local function aimAtTarget(targetPlayer)
    ...
    -- Fallback jika Head tidak ditemukan
    if not targetPart then
        targetPart = targetPlayer.Character:FindFirstChild("HumanoidRootPart") or targetPlayer.Character:FindFirstChild("Torso")
    ...
    if aimbotConfig.crosshairTargeting then
        ...
        -- Hitung offset dinamis berdasarkan jarak
        local headOffset = calculateDynamicHeadOffset(distance)
        
        -- Add offset untuk kepala (dinamis berdasarkan jarak)
        if targetPart.Name == "Head" and targetPart.Size then
            targetPosition = targetPosition + Vector3.new(0, targetPart.Size.Y * headOffset, 0)
        end

        -- TAMBAH EXTRA HEIGHT DARI CONFIG
        targetPosition = targetPosition + Vector3.new(0, aimbotConfig.extraHeight, 0)
        
        local screenPosition, onScreen = Camera:WorldToViewportPoint(targetPosition)
        if not onScreen then return end
        
        -- Jangan gerakkan mouse jika klik kanan ditekan
        if aimbotConfig.rightMouseDown then
            return
        end
        ...
        -- Apply smoothing untuk pergerakan yang lebih natural
        ...
        -- Add offset untuk kepala
        if targetPart.Name == "Head" and targetPart.Size then
            targetPosition = targetPosition + Vector3.new(0, targetPart.Size.Y * headOffset, 0)
        end
        
        -- TAMBAH EXTRA HEIGHT DARI CONFIG JUGA UNTUK MODE LOCK
        targetPosition = targetPosition + Vector3.new(0, aimbotConfig.extraHeight, 0)
...
local function toggleAimbot()
    ...
    if aimbotConfig.enabled then
        startAimbot()
        print("Aimbot: ON - Mode:", aimbotConfig.crosshairTargeting and "Crosshair Targeting (Unlock Mouse)" or "Camera Targeting (Lock Mouse)")
        print("Target: Head (Fixed)")
    else
        stopAimbot()
        print("Aimbot: OFF")
    end
...
-- UI Controls untuk pengaturan offset dinamis
CombatTab:CreateToggle({
    Name = "Dynamic Head Offset (Rekomended)",
    CurrentValue = true,
    Callback = function(Value)
        aimbotConfig.dynamicHeadOffset = Value
        print("Dynamic Head Offset:", Value and "ON" or "OFF")
    end,
})

CombatTab:CreateSlider({
    Name = "Base Head Offset (Jarak Dekat)",
    Range = {0.5, 1.0},
    Increment = 0.05,
    Suffix = "Height",
    CurrentValue = 0.8,
    Callback = function(Value)
        aimbotConfig.baseHeadOffset = Value
        print("Base Head Offset (Dekat):", Value)
    end,
})

CombatTab:CreateSlider({
    Name = "Max Head Offset (Jarak Jauh)",
    Range = {0.1, 0.5},
    Increment = 0.05,
    Suffix = "Height",
    CurrentValue = 0.3,
    Callback = function(Value)
        aimbotConfig.maxHeadOffset = Value
        print("Max Head Offset (Jauh):", Value)
    end,
})

CombatTab:CreateSlider({
    Name = "Extra Height Above Head",
    Range = {0, 2},
    Increment = 0.1,
    Suffix = "Studs",
    CurrentValue = 0.5,
    Callback = function(Value)
        aimbotConfig.extraHeight = Value
        print("Extra Height:", Value)
    end,
})
...
CombatTab:CreateToggle({
    Name = "Crosshair Targeting (Unlock Mouse)",
    CurrentValue = true,
    Callback = function(Value)
        aimbotConfig.crosshairTargeting = Value
        print("Crosshair Targeting:", Value and "ON (Unlock Mouse - Hold RMB to move camera)" or "OFF (Lock Mouse)")
    end,
})
...
local function setupDoubleDamage()
    -- Hook into weapon damage systems
    local function hookWeaponDamage()
        -- Hook RemoteEvents for weapon damage
                        ...
                        for i, arg in pairs(args) do
                            ...
                            elseif type(arg) == "table" then
                                ...
                                for key, value in pairs(arg) do
                                    if type(value) == "number" and value > 0 and value < 1000 then
                                        local keyLower = tostring(key):lower()
                                        if keyLower:find("damage") or keyLower:find("dmg") or keyLower:find("amount") or 
                                           keyLower:find("health") or keyLower:find("hp") or keyLower:find("hit") or
                                           keyLower:find("force") or keyLower:find("power") then
        ...
        -- Hook RemoteFunctions for weapon damage
        local function hookDamageFunction(func)
        ...
        if game.ReplicatedStorage then
            game.ReplicatedStorage.DescendantAdded:Connect(onAdded)
        end
        workspace.DescendantAdded:Connect(onAdded)
        if game.Players then
            game.Players.DescendantAdded:Connect(onAdded)
        end
    end
